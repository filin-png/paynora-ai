...
| Remaining manual steps | Create/reuse an Anthropic API key with web search enabled and set the two env vars. **Known limitation**: no existing AI-conversational call site in this codebase currently decides *automatically* "should I search the web" — the Phase 12 Intelligence MVP (Risk Copilot) that would have been the natural place to wire that decision loop into was never merged to `main` in this branch. `tryWebSearch`/`runDeepResearch` are complete, real, and fully tested as standalone capabilities, ready to be called from that decision point whenever it exists; wiring an automatic trigger is future work, not a gap in what was built this phase. `yandex` remains recognized-but-not-implemented (research confirmed pricing/quota but not enough live-verified request/response detail to implement confidently without risking a wrong implementation). |
